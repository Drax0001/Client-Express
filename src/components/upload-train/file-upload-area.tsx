...
import { Upload, FileText, X, AlertCircle, CheckCircle, FileIcon } from "lucide-react"
...
export function FileUploadArea({
  ...
  const validateFile = (file: File): { isValid: boolean; errors: string[] } => {
    ...
    if (file.size > maxFileSize) {
      errors.push(`File size (${formatFileSize(file.size)}) exceeds limit (${formatFileSize(maxFileSize)})`)
    }

    // Check file type
    const fileExtension = file.name.split('.').pop()?.toLowerCase()
    if (!fileExtension || !supportedTypes.includes(fileExtension)) {
      errors.push(`File type not supported. Supported: ${supportedTypes.join(', ')}`)
    }
