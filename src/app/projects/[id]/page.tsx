      ...
      <MainLayout>
        <div className="flex items-center justify-center min-h-[50vh]">
          <div className="flex flex-col items-center gap-4 text-muted-foreground">
            <AppIcon name="Loader2" className="h-8 w-8 animate-spin text-brand" />
  ...
  const handleSendMessage = async (content: string) => {
    ...
    try {
      ...
      const assistantMessage: ChatMessage = {
        ...
        // Since the current API might lack full source snippet mapping, mock it for the UI preview or use what's available
        sources: response.sourceCount > 0 ? Array.from({ length: response.sourceCount }).map((_, i) => ({
          id: `src-${i}`,
          title: `Project Document ${i + 1}`,
          snippet: "Snippet extraction is currently handled by the backend API...",
      ...
      <div className="flex flex-col h-full overflow-hidden w-full max-w-6xl mx-auto animate-in fade-in duration-300">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-3">
                ...
                <Badge variant={hasDocuments ? "default" : "secondary"} className={hasDocuments ? "bg-green-500/10 text-green-500 hover:bg-green-500/20" : ""}>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col min-h-0">
          ...
          <div className="flex-1 min-h-0 relative mt-6">
            ...
            {/* CHAT TAB */}
            <TabsContent value="chat" className="h-full m-0 data-[state=inactive]:hidden focus-visible:outline-none flex gap-6">
              <div className="flex-1 border border-border/60 rounded-xl bg-card shadow-sm overflow-hidden flex flex-col min-h-[500px]">
                {!hasDocuments ? (
                  <div className="flex-1 flex flex-col items-center justify-center p-8 text-center text-muted-foreground bg-background/50">
                    ...
                    <h3 className="text-xl font-medium text-foreground mb-2">No Knowledge Sources</h3>
                    <p className="max-w-sm mb-6">
                      Your chatbot doesn't know anything yet. It needs documents or web links to accurately answer questions.
                    </p>
            ...
            {/* SOURCES TAB */}
            <TabsContent value="sources" className="h-full m-0 data-[state=inactive]:hidden focus-visible:outline-none space-y-6 overflow-y-auto pb-10">
              <Card className="border-border/60 shadow-sm">
                <CardHeader className="bg-muted/30 border-b border-border/60">
                  <CardTitle className="text-lg">Add New Sources</CardTitle>
                  <CardDescription>Upload files or import URLs to expand the chatbot's knowledge.</CardDescription>
                </CardHeader>
              ...
              <Card className="border-border/60 shadow-sm">
                <CardHeader className="bg-muted/30 border-b border-border/60 py-4 flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription>Manage the files currently powering this chatbot.</CardDescription>
                  </div>
                  <Badge variant="secondary">{project.documents?.length || 0} Total</Badge>
                </CardHeader>
...
<script src="https://cdn.example.com/widget.js" defer></script>`);
              ...
              <div className="flex flex-col gap-6">
                <Card className="border-border/60 shadow-sm">
                  <CardHeader>
                    ...
                    <CardDescription>Number of messages processed by this chatbot over the last 7 days.</CardDescription>
                  </CardHeader>
                  <CardContent className="h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={mockLineData}>
                        ...
                        <Line type="monotone" dataKey="messages" stroke="hsl(var(--brand))" strokeWidth={3} dot={{ fill: "hsl(var(--brand))", r: 4 }} activeDot={{ r: 6 }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </CardContent>
                </Card>
                ...
                <Card className="border-border/60 shadow-sm">
                  <CardHeader>
                    ...
                    <CardDescription>Most frequently discussed topics based on semantic clustering.</CardDescription>
                  </CardHeader>
                  <CardContent className="h-[300px]">
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={mockBarData} layout="vertical" margin={{ left: 20 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" horizontal={false} />
                        <XAxis type="number" stroke="hsl(var(--muted-foreground))" fontSize={12} tickLine={false} axisLine={false} />
                        <YAxis dataKey="name" type="category" stroke="hsl(var(--muted-foreground))" fontSize={12} tickLine={false} axisLine={false} />
                        <Tooltip cursor={{ fill: "hsl(var(--muted))" }} contentStyle={{ backgroundColor: "hsl(var(--card))", borderColor: "hsl(var(--border))", borderRadius: "8px" }} />
                        <Bar dataKey="requests" fill="hsl(var(--brand))" radius={[0, 4, 4, 0]} />
                      </BarChart>
                    </ResponsiveContainer>
                  </CardContent>
                </Card>
            ...
            {/* SETTINGS TAB */}
            <TabsContent value="settings" className="h-full m-0 data-[state=inactive]:hidden focus-visible:outline-none max-w-2xl">
              <Card className="border-border/60 shadow-sm mb-6">
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">Name modification is disabled for now.</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="border-destructive/30 shadow-sm">
                ...
                <CardContent>
                  <div className="flex items-center justify-between border border-destructive/20 rounded-lg p-4 bg-destructive/5">
                    <div>
                      <h4 className="font-medium text-destructive">Delete Chatbot</h4>
                      <p className="text-sm text-destructive/80">
                        Permanently delete this project, its documents, and all chat history.
                      </p>
                    </div>
